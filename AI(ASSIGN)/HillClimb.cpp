           ...
           int final[N][N])
{
    ...
    // visited.insert(root); // calculate the cost of the root node or the initial configuration
    ...
    while (!pq.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (isSafe(currentState->x + row[i], currentState->y + col[i]))
            {
                ...
                if (calculateCost(child->mat, final) > calculateCost(neighbour->mat, final))
                {
                    // cout << "Inside the else if block\n";
                    neighbour = child;
                    cost = calculateCost(child->mat, final);
                    flag = true;
                }
            }
        }
